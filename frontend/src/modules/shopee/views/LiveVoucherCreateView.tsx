...
interface LiveVoucherCreateViewProps {
  runId: number | null;
  readOnly?: boolean;
...
const voucherIllustrationUrl = 'https://deo.shopeemobile.com/shopee/shopee-seller-live-sg/mmf_portal_seller_root_dir/static/modules/vouchers-v2/image/voucher_live_stream_preview_th.777d61b.png';

export default function LiveVoucherCreateView({ runId, readOnly = false, detailCampaignId = null, onBackToVouchers }: LiveVoucherCreateViewProps) {
  ...
  useEffect(() => {
    ...
    const endpoint = detailCampaignId
      ? `${API_BASE_URL}/shopee/runs/${runId}/marketing/vouchers/detail/live_voucher/${detailCampaignId}`
      : `${API_BASE_URL}/shopee/runs/${runId}/marketing/vouchers/live-create/bootstrap`;
    fetch(endpoint, {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const params = new URLSearchParams({ voucher_type: 'live_voucher', code_suffix: suffix });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/shopee/runs/${runId}/marketing/vouchers/live-create/eligible-products?${params.toString()}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleConfirmPicker = () => {
    const next = Object.values(pickerSelections);
    setSelectedProducts(next.slice(0, 100));
    setPickerOpen(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/shopee/runs/${runId}/marketing/vouchers/live-campaigns`, {
        method: 'POST',
        ...
        <div className="flex items-start gap-6 shrink-0 h-full">
            ...
            <div className="flex flex-col gap-4 pb-6">
                ...
                <div className="grid grid-cols-[180px_1fr] items-start gap-y-6 text-[14px]">
                  ...
                  <div>
                    <div className="relative flex h-[40px] w-[180px] items-center justify-center rounded-sm border border-[#ee4d2d] bg-white text-[#ee4d2d] cursor-pointer">
                      <svg viewBox="0 0 24 24" className="mr-2 h-[18px] w-[18px]" fill="currentColor"><path d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4z"/></svg>
                      直播代金券
                  ...
                  <div className="max-w-[700px]">
                    ...
                    <div className="mt-2 text-[12px] leading-relaxed text-[#999]">
                      请输入 A-Z, 0-9; 最多 5 个字符。<br />您的完整代金券代码为: {codePrefix}{codeSuffix}
              ...
              <section className="border border-[#ececec] bg-white px-8 py-7 shadow-sm rounded-[2px]">
                ...
                <div className="grid grid-cols-[180px_1fr] items-start gap-y-6 text-[14px]">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="liveRewardType" checked readOnly className="h-4 w-4 accent-[#ee4d2d]" /><span>折扣</span></label>
                  ...
                  <div className="flex h-9 w-[452px] items-center rounded-sm border border-[#e5e5e5] focus-within:border-[#ee4d2d] overflow-hidden"><div className="bg-[#f5f5f5] px-3 h-full flex items-center border-r border-[#e5e5e5] text-[#999]">{currency}</div><input type="number" value={minSpendAmount} onChange={(e) => setMinSpendAmount(e.target.value)} disabled={effectiveReadOnly} className="flex-1 outline-none text-[14px] px-3 disabled:bg-white" /></div>
              ...
              <section className="border border-[#ececec] bg-white px-8 py-7 shadow-sm rounded-[2px]">
                ...
                <div className="grid grid-cols-[180px_1fr] items-start gap-y-6 text-[14px]">
                  ...
                  <div className="text-[#333] pt-1">仅在 Shopee Live 中展示</div>
          ...
          <div className="w-[320px] shrink-0 sticky top-0">
            <div className="border border-[#ececec] bg-white p-5 shadow-sm rounded-[2px]">
              ...
              <div className="mt-4 text-[11px] text-[#999] text-center px-4">买家只能将此代金券用于从 Shopee Live 添加的商品。</div>
            </div>
          </div>
        </div>
      </div>
