...
interface PrivateVoucherCreateViewProps {
  runId: number | null;
  readOnly?: boolean;
...
interface VoucherDetailResponse extends VoucherCreateBootstrapResponse {
  ...
  form: VoucherCreateBootstrapResponse['form'] & {
...
export default function PrivateVoucherCreateView({ runId, readOnly = false, detailCampaignId = null, onBackToVouchers }: PrivateVoucherCreateViewProps) {
  ...
  useEffect(() => {
    ...
    const endpoint = detailCampaignId
      ? `${API_BASE_URL}/shopee/runs/${runId}/marketing/vouchers/detail/private_voucher/${detailCampaignId}`
      : `${API_BASE_URL}/shopee/runs/${runId}/marketing/vouchers/private-create/bootstrap`;
    fetch(endpoint, {
      ...
      .then((result) => {
        ...
        setApplicableProductType('applicable_scope' in result.form && result.form.applicable_scope === 'selected_products' ? 'specific' : 'all');
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const params = new URLSearchParams({ voucher_type: 'private_voucher', code_suffix: suffix });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/shopee/runs/${runId}/marketing/vouchers/private-create/eligible-products?${params.toString()}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      const response = await fetch(`${API_BASE_URL}/shopee/runs/${runId}/marketing/vouchers/private-campaigns`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          selected_products: applicableProductType === 'specific' ? selectedProducts.flatMap((item) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#f6f6f6] custom-scrollbar text-[#333] flex flex-col">
      <div className="mx-auto w-full max-w-[1360px] px-6 pt-6 pb-6 flex-1 flex flex-col relative">
                ...
                <div className="grid grid-cols-[180px_1fr] items-start gap-y-6 text-[14px]">
                  ...
                  <div className="flex items-center gap-6">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="privateVoucherRewardType" checked readOnly className="h-4 w-4 accent-[#ee4d2d]" />
                      <span>折扣</span>
                    </label>
                  </div>
              ...
              <section className="border border-[#ececec] bg-white px-8 py-7 shadow-sm rounded-[2px]">
                ...
                <div className="grid grid-cols-[180px_1fr] items-start gap-y-8 text-[14px]">
                  ...
                  <div className="flex flex-col gap-4 max-w-[760px]">
                    <div>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="applicableProduct" checked={applicableProductType === 'all'} onChange={() => setApplicableProductType('all')} disabled={effectiveReadOnly} className="h-4 w-4 accent-[#ee4d2d]" />
                        <span>全部商品</span>
                      </label>
                      {applicableProductType === 'all' && (
                        <div className="ml-6 mt-1 text-[12px] text-[#eda500] leading-relaxed max-w-[600px]">
                          通过专属代码分享，买家可在店铺全部可参与商品中使用。
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="applicableProduct" checked={applicableProductType === 'specific'} onChange={() => setApplicableProductType('specific')} disabled={effectiveReadOnly} className="h-4 w-4 accent-[#ee4d2d]" />
                        <span>指定商品</span>
                      </label>
                        ...
                        <div className="ml-6 mt-4">
                          {selectedProducts.length > 0 ? (
                            ...
                            <div className="mb-3 flex items-center gap-4">
                              <span className="text-[14px] text-[#333]">商品</span>
                              <button type="button" onClick={handleOpenPicker} disabled={effectiveReadOnly} className="inline-flex h-8 items-center gap-1.5 rounded-sm border border-[#ee4d2d] px-4 text-[13px] text-[#ee4d2d] hover:bg-[#fff6f4] disabled:opacity-50 disabled:cursor-not-allowed">
                                <PlusIcon />
                                添加商品
                              </button>
