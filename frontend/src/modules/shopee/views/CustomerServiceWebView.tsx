...
export default function CustomerServiceWebView({ runId, readOnly = false }: CustomerServiceWebViewProps) {
  ...
  const scoreDetail = detail?.score_detail;
  const isDamageRefund = detail?.scenario_code === 'delivered_damage_refund' && scoreDetail?.resolution_type;
  const currentScenarioLabel = detail ? (scenarioLabels[detail.scenario_code] ?? '客服咨询') : '请选择会话';
  const resolveOrderStatusLabel = (order: any) => order?.type_bucket === 'shipping' ? '运送中' : (order?.process_status || '订单');

  useEffect(() => {
    const order = detail?.order_context?.order;
    if (order) setActiveOrderStatus(resolveOrderStatusLabel(order));
  }, [detail?.id, detail?.order_context?.order?.type_bucket, detail?.order_context?.order?.process_status]);
  ...
  const noOrderConversations = conversations.filter((item) => item.scenario_code === 'product_detail_inquiry');
  ...
  const hasNoOrderUnreadMessages = noOrderConversations.some((item) => Number(item.unread_count ?? 0) > 0);
    ...
    return (
      <div className="flex flex-col h-full bg-[#f6f6f6] p-3 overflow-y-auto">
        <div className="bg-white rounded border border-gray-200 p-3 shadow-sm">
          ...
          {isDamageRefund ? <div className="mt-3 pt-3 border-t border-gray-100 text-[12px] text-gray-700 space-y-1"><div>处理方式：{resolutionLabels[scoreDetail.resolution_type] ?? scoreDetail.resolution_type}</div>{(scoreDetail.commentary || scoreDetail.summary) && <div>{scoreDetail.commentary || scoreDetail.summary}</div>}{Array.isArray(scoreDetail.suggestions) && scoreDetail.suggestions.length > 0 && <ul className="list-disc pl-4 space-y-0.5">{scoreDetail.suggestions.map((item: string, index: number) => <li key={index}>{item}</li>)}</ul>}</div> : detail?.satisfaction_score != null && <div className="mt-3 pt-3 border-t border-gray-100 text-[12px] text-gray-700 space-y-1"><div>满意度：{detail.satisfaction_score}（{detail.satisfaction_level}）</div>{(scoreDetail?.commentary || scoreDetail?.summary) && <div>点评：{scoreDetail.commentary || scoreDetail.summary}</div>}{Array.isArray(scoreDetail?.suggestions) && scoreDetail.suggestions.length > 0 && <ul className="list-disc pl-4 space-y-0.5">{scoreDetail.suggestions.map((item: string, index: number) => <li key={index}>{item}</li>)}</ul>}</div>}
        </div>
      </div>
    );
  };
  ...
  const renderOrderTab = () => {
    const orderContext = detail?.order_context;
    const order = orderContext?.order;
    const logistics = orderContext?.logistics;
    const items = Array.isArray(orderContext?.items) ? orderContext.items : [];
    const statusLabel = resolveOrderStatusLabel(order);
    const trackingNo = order?.tracking_no || order?.waybill_no || '-';
    return (
      <div className="flex flex-col h-full bg-[#f6f6f6]">
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {!order ? (
            <div className="h-full flex items-center justify-center text-[13px] text-gray-400">当前会话暂无关联订单</div>
          ) : (
            <div className="bg-white rounded border border-gray-200 p-3 shadow-sm text-[12px] text-gray-700 space-y-3">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center min-w-0"><CheckCircle2 size={14} className="mr-1.5 text-[#26aa99] flex-shrink-0" /><span className="font-medium text-gray-900 truncate">{statusLabel}</span></div>
                <div className="flex items-center text-gray-400 min-w-0"><span className="truncate">{order.order_sn}</span><Copy size={12} className="ml-1 opacity-70 flex-shrink-0" /></div>
              </div>
              <div className="grid grid-cols-2 gap-x-3 gap-y-1">
                <span className="text-gray-400">买家</span><span className="text-right truncate">{order.buyer_name || '-'}</span>
                <span className="text-gray-400">物流渠道</span><span className="text-right truncate">{order.shipping_channel || '-'}</span>
                <span className="text-gray-400">追踪号</span><span className="text-right truncate">{trackingNo}</span>
                <span className="text-gray-400">发货时间</span><span className="text-right">{formatDate(order.shipped_at)} {formatTime(order.shipped_at)}</span>
                <span className="text-gray-400">预计送达</span><span className="text-right">{formatDate(order.eta_start_at)} - {formatDate(order.eta_end_at)}</span>
                <span className="text-gray-400">买家付款</span><span className="text-right text-[#ee4d2d] font-medium">RM {order.buyer_payment ?? 0}</span>
              </div>
              {logistics && <div className="pt-3 border-t border-gray-100 space-y-1">
                <div className="font-medium text-gray-900">最新物流</div>
                <div>{logistics.last_event_title || '已发货'}</div>
                <div className="text-gray-500 leading-relaxed">{logistics.last_event_desc || '订单正在运输中'}</div>
                <div className="flex justify-between text-gray-400"><span>{formatDate(logistics.last_event_time)} {formatTime(logistics.last_event_time)}</span><span>停滞 {logistics.stalled_game_hours ?? 0} 游戏小时</span></div>
              </div>}
              {items.length > 0 && <div className="pt-3 border-t border-gray-100 space-y-2">
                <div className="font-medium text-gray-900">商品明细</div>
                {items.map((item: any, index: number) => <div key={`${item.listing_id}-${item.variant_id ?? index}`} className="flex gap-2">
                  <div className="w-10 h-10 bg-gray-100 rounded overflow-hidden flex-shrink-0">{item.image_url && <img src={item.image_url} alt="Product" className="w-full h-full object-cover" />}</div>
                  <div className="flex-1 min-w-0"><div className="truncate text-gray-800">{item.product_name}</div><div className="text-gray-400 truncate">{item.variant_name || '默认规格'} × {item.quantity}</div></div>
                  <div className="text-[#ee4d2d] font-medium">RM {item.unit_price ?? 0}</div>
                </div>)}
              </div>}
            </div>
          )}
