# 39-Shopee 直播代金券创建前后端数据库 Redis 实现设计
...
为 Shopee 营销中心代金券模块接入“直播代金券”创建能力，使 `/shopee/marketing/vouchers/live-create` 页面从前端静态表单升级为可读取初始化数据、校验输入、选择适用商品、提交创建并落库的完整流程。

本期重点是接口、数据库和 Redis 数据闭环：

- 前端样式和布局沿用用户已手动调整后的 `LiveVoucherCreateView.tsx`，不重做视觉结构。
...
- 后端新增直播代金券创建页 bootstrap、可选商品、创建提交接口，并扩展代金券列表与代码检查兼容 `live_voucher`。
- 数据库新增直播代金券活动表与适用商品表，保存影响经营结果的代金券规则、直播展示范围和商品快照。
- Redis 用于创建页 bootstrap、商品选择列表、代码检查、代金券列表缓存、创建限流和创建后缓存失效。
- 买家端直播间领券、直播场景下单命中与订单归因先预留字段，不在本期实现。

## 2. 范围与非范围

### 2.1 本期范围

- 直播代金券类型：实现 `live_voucher`。
- 前端 `/shopee/marketing/vouchers/live-create` 接口对接：
  - 初始化表单默认值。
...
- 不重做直播代金券创建页前端布局、颜色、间距、右侧预览图和基础表单视觉。
- 不接入真实 Shopee 平台 API。
- 不实现直播间真实开播、直播商品讲解、买家端领券入口或直播观看行为。
- 不实现直播代金券详情、编辑、复制、停止、删除和数据页。
- 不在本期改订单模拟下单概率、代金券命中、价格改写或订单归因；仅预留后续接入口径。
- 不实现上传商品列表的真实解析流程；上传 Tab 如复用弹窗，本期保留界面占位。

## 3. 游戏时间口径

直播代金券所有面向前端的时间字段必须统一使用游戏时间：
...
- `LiveVoucherCreateView.tsx` 的 `DateTimePicker` 初始值必须来自 bootstrap 返回的 `form.start_at`、`form.end_at` 和 `form.display_start_at`。
...
直播代金券用于 Shopee Live 场景。买家只能在直播场景中获得或使用该代金券，V1 创建流程只保存规则和商品范围，后续直播间领券与订单命中统一接入。

| 字段 | 规则 |
| --- | --- |
| 代金券类型 | 固定为 `live_voucher` |
...
| 展示设置 | V1 固定 `live_stream`，前端文案为“仅在 Shopee Live 中展示” |
| 适用商品 | 支持 `all_products` / `selected_products`；指定商品模式至少选择 1 个商品 |
| 直播范围 | V1 预留 `live_scope=all_live_sessions`，不绑定具体直播场次 |

### 4.2 适用商品规则

- `applicable_scope=all_products`：直播代金券适用于当前卖家同一 `run_id/user_id` 下所有可售商品；创建时不写商品明细。
...
- 命中条件：订单属于同一 `run_id/user_id` 店铺，订单游戏时间在代金券使用期限内，买家来源或商品来源命中 Shopee Live 场景，订单商品命中适用范围，订单金额满足 `min_spend_amount`。
- `all_products`：订单内所有直播场景可售商品小计可参与优惠。
...
- 买家限用：同一买家对同一直播代金券累计使用次数不超过 `per_buyer_limit`。
- 归因字段：后续订单级写入 `marketing_campaign_type='live_voucher'`、`marketing_campaign_id`、`marketing_campaign_name_snapshot`。

## 5. 数据模型设计

### 5.1 新增 ORM：`ShopeeLiveVoucherCampaign`

建议新增表：`shopee_live_voucher_campaigns`。

> 数据库 Schema Rules 要求：新建表必须添加表注释，新增字段必须添加字段注释。

| 字段 | 类型 | 约束 | 注释 |
| --- | --- | --- | --- |
| `id` | Integer | PK | 直播代金券活动 ID |
| `run_id` | Integer | FK `game_runs.id`, index | 所属对局 ID |
| `user_id` | Integer | FK `users.id`, index | 所属卖家用户 ID |
| `voucher_type` | String(32) | not null, default `live_voucher` | 代金券类型，V1 固定直播代金券 |
| `voucher_name` | String(255) | not null | 卖家可见的代金券名称 |
| `voucher_code` | String(32) | not null | 完整代金券代码，如 `HOMELIVE1` |
...
| `display_type` | String(32) | not null, default `live_stream` | 展示方式，直播代金券 V1 固定直播间展示 |
| `display_channels` | Text | nullable | 展示渠道配置 JSON，V1 固定 `["shopee_live"]` |
...
| `live_scope` | String(32) | not null, default `all_live_sessions` | 直播适用范围，V1 固定全部直播场次 |
| `live_payload` | Text | nullable | 直播场次绑定配置 JSON，V1 预留为空 |
...
### 5.2 新增 ORM：`ShopeeLiveVoucherItem`

建议新增表：`shopee_live_voucher_items`。

| 字段 | 类型 | 约束 | 注释 |
| --- | --- | --- | --- |
| `id` | Integer | PK | 直播代金券适用商品记录 ID |
| `campaign_id` | Integer | FK `shopee_live_voucher_campaigns.id`, index | 直播代金券活动 ID |
| `run_id` | Integer | FK `game_runs.id`, index | 所属对局 ID |
| `user_id` | Integer | FK `users.id`, index | 所属卖家用户 ID |
...
```http
GET /shopee/runs/{run_id}/marketing/vouchers/live-create/bootstrap
```

用途：为直播代金券创建页提供游戏时间默认值、表单规则、只读状态和商品选择配置。
...
```http
GET /shopee/runs/{run_id}/marketing/vouchers/live-create/eligible-products
```
...
```http
GET /shopee/runs/{run_id}/marketing/vouchers/code/check?voucher_type=live_voucher&code_suffix=LIVE1
```

要求：

- `voucher_type` 支持 `live_voucher`。
- 完整代码仍按 `HOME + code_suffix` 生成。
- 同一 `run_id/user_id` 下，店铺代金券、商品代金券、专属代金券、直播代金券之间代码不可重复。
- 结果写入 Redis 短缓存。

### 6.4 创建直播代金券

```http
POST /shopee/runs/{run_id}/marketing/vouchers/live-campaigns
```
...
- 历史对局只读时禁止创建。
- `voucher_type` 强制为 `live_voucher`，不信任前端传入其他类型。
- `display_type` 强制为 `live_stream`，`display_channels` 强制为 `["shopee_live"]`。
- `voucher_name` 必填且不超过 100 字符。
- `code_suffix` 仅允许 `A-Z0-9`，最多 5 字符。
...
1. 获取 Redis 创建限流锁。
2. 解析并校验请求。
3. 生成完整 `voucher_code = HOME + code_suffix`。
4. 检查同一 `run_id/user_id` 下所有代金券代码唯一。
5. 创建 `ShopeeLiveVoucherCampaign`。
6. 若指定商品，批量创建 `ShopeeLiveVoucherItem` 快照。
7. 提交事务。
8. 清理代金券列表、直播代金券创建页和可选商品相关缓存。
9. 返回创建结果。
...
`GET /shopee/runs/{run_id}/marketing/vouchers` 需要合并返回直播代金券：

- `voucher_type=live_voucher`
- `voucher_type_label=直播代金券`
- `display_type_label=Shopee Live`
- `period_text` 使用游戏时间。
- `products` 使用 `shopee_live_voucher_items` 商品快照；全部商品时展示“全部商品”。

## 7. 前端接口接入设计

### 7.1 页面状态

`LiveVoucherCreateView.tsx` 保留现有布局，在现有 state 基础上补充：

- `runId: number | null` prop，用于调用后端接口。
- `loading/saving/error/serverReadOnly`。
- `codeSuffix/codeCheck/codeChecking`。
...
```ts
GET /shopee/runs/${runId}/marketing/vouchers/live-create/bootstrap
```
...
```ts
GET /shopee/runs/${runId}/marketing/vouchers/live-create/eligible-products
```
...
当前 `LiveVoucherCreateView.tsx` 尚未展示代码输入区域时，后续实现可按现有店铺/商品/专属代金券口径补充。

接口：

```ts
GET /shopee/runs/${runId}/marketing/vouchers/code/check?voucher_type=live_voucher&code_suffix=${suffix}
```
...
- debounce 300ms。
- 显示可用/不可用提示。
- 提交前若 `codeCheck.available=false` 则阻止创建。
...
```ts
POST /shopee/runs/${runId}/marketing/vouchers/live-campaigns
```
...
| Key | TTL | 用途 |
| --- | --- | --- |
| `shopee:vouchers:live:create:bootstrap:{run_id}:{user_id}` | 5 分钟 | 直播代金券创建页初始化数据 |
| `shopee:vouchers:live:create:eligible:{run_id}:{user_id}:{hash}` | 2 分钟 | 可选商品列表 |
| `shopee:vouchers:code-check:{run_id}:{user_id}:{voucher_type}:{code_suffix}` | 60 秒 | 代码可用性检查 |
| `shopee:vouchers:list:{run_id}:{user_id}:{hash}` | 2 分钟 | 代金券列表缓存，需兼容直播代金券 |
| `shopee:vouchers:live:create:limit:{run_id}:{user_id}` | 10 秒 | 创建接口限流 |
...
- `shopee:vouchers:live:create:bootstrap:{run_id}:{user_id}`
- `shopee:vouchers:live:create:eligible:{run_id}:{user_id}:*`
- `shopee:vouchers:code-check:{run_id}:{user_id}:live_voucher:*`
...
- 访问 `/shopee/marketing/vouchers/live-create` 时，页面保持用户现有直播代金券创建页布局。
...
- `POST /live-campaigns` 创建成功后，活动和商品明细落库，缓存正确失效，并返回代金券列表。
- `/shopee/marketing/vouchers` 列表可展示直播代金券。
- 历史对局回溯模式下可浏览创建页，但无法创建。
- 后端新增表和字段均补齐表注释、字段注释、索引和唯一约束。

## 11. 后续扩展

- 接入直播间真实领券、直播间商品曝光和买家观看行为。
- 接入订单模拟命中直播代金券、优惠金额改写和归因统计。
- 增加直播代金券详情、编辑、复制、停止、删除和数据页。
- 上传商品列表解析和批量选择。
- 直播场次维度的代金券绑定，如 `live_scope=specific_sessions`。
